...
<div class="main-content">
    ...
    <main class="page-content">
        ...
        <div class="tabs">
            <a href="?tab=pending" class="tab <?php echo $active_tab === 'pending' ? 'active' : ''; ?>">Pending
                (<?php echo count($pending_bills); ?>)</a>
            <a href="?tab=paid" class="tab <?php echo $active_tab === 'paid' ? 'active' : ''; ?>">Paid</a>
            <a href="?tab=create" class="tab <?php echo $active_tab === 'create' ? 'active' : ''; ?>">Create New
                Bill</a>
        </div>
        ...
        <?php if ($active_tab === 'pending'): ?>
                        ...
                        <tbody>
                            <?php foreach ($pending_bills as $b): ?>
                                <tr>
                                    ...
                                    <td><?php echo h($b['patient_name']); ?><br><span
                                            class="text-sm text-gray"><?php echo h($b['patient_code']); ?></span></td>
                                    ...
                                    <td style="color: #dc2626; font-weight: 600;">
                                        $<?php echo number_format($b['due_amount'], 2); ?></td>
                                    <td>
                                    ...
                                    <td>
                                        <button class="btn btn-sm btn-primary"
                                            onclick="showPaymentModal('<?php echo $b['id']; ?>', <?php echo $b['due_amount']; ?>)">💳
                                            Pay</button>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                            <?php if (empty($pending_bills)): ?>
                                <tr>
                                    <td colspan="7" class="text-center text-gray">No pending bills.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
            ...
            <!-- Payment Modal -->
            <div id="paymentModal" class="card"
                style="display: none; position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); z-index: 1000; width: 400px; box-shadow: 0 4px 20px rgba(0,0,0,0.3);">
                <div class="card-header">
                        ...
                        <tbody>
                            <?php foreach ($paid_bills as $b): ?>
                                <tr>
                                    ...
                                    <td><a href="receipt.php?bill_id=<?php echo $b['id']; ?>" target="_blank"
                                            class="btn btn-sm btn-outline">View Receipt</a></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
                    ...
                    <div id="billItems">
                        <div class="grid-2" style="gap: 8px; margin-bottom: 8px;">
                            <input type="text" name="items[0][description]" class="form-input"
                                placeholder="Description (e.g., Consultation Fee)" required>
                            ...
                            <input type="number" name="items[0][quantity]" class="form-input" placeholder="Qty" value="1"
                                style="width: 80px;">
                            <input type="number" step="0.01" name="items[0][unit_price]" class="form-input"
