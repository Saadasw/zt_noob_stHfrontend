...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($email) || empty($password)) {
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            // If staff, fetch department from staff_profiles
            if ($user['role'] === 'staff') {
                $stmt = $pdo->prepare("SELECT department, branch_id FROM staff_profiles WHERE user_id = ?");
                $stmt->execute([$user['id']]);
                $staffProfile = $stmt->fetch();
                if ($staffProfile) {
                    $_SESSION['staff_department'] = $staffProfile['department'];
                    $_SESSION['staff_branch_id'] = $staffProfile['branch_id'];
                }
            }
